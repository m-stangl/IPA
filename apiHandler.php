...
} elseif (isset($_POST["task"]) && $_POST["task"] == "getTags") {

  //Tag-Nummer auslesen, da sie der Funktion übergeben werden muss
  $tagNr = $_POST["tagNr"];
...
function getDetails($access_token, $serialNr)
{
  ...
  if (isset($responseArray[0]->id)) {
    ...
    echo "Die gesuchte Seriennummer existiert nicht";
  }
}
...
function getAsset($access_token, $assetNr)
{
  ...
  if ($assetArray->count !== 0) {
    ...
    usort($assetArray->articlesIt, "cmp");
    usort($assetArray->articlesExtern, "cmp");
    usort($assetArray->articlesIwc, "cmp");
    ...
    echo "Die gesuchte Anlagenummer existiert nicht";
  }
}
